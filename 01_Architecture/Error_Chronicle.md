...
## Session 2026-07-05 -- OpSec-Audit-Erweiterung v2.1 (Checks 35-38)

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 104 | OpSec-Audit Check 23 (NetBIOS) meldete pauschal [X] ueber ALLE NetBT-Interfaces, ohne aktive von inaktiven zu unterscheiden -- gleiches False-Negative-Muster wie zuvor bei Check 04 (IPv6) | LOGIK | Der Check brach beim ersten Interface mit NetbiosOptions!=2 ab, ohne zu pruefen ob es einem verbundenen Adapter entspricht. Ein toter Adapter mit =0 haette faelschlich [X] ausgeloest | Check 23 v2.1: gleicht NetBT-Interface-GUIDs gegen aktive Adapter (Status=Up) ab, wertet nur AKTIVE nicht-deaktivierte Interfaces als Problem und nennt deren Anzahl. Ergebnis blieb [X], aber aus dem richtigen Grund (2 echte aktive vEthernet-Switches) | Interface-basierte Checks (NetBIOS, IPv6, DNS) IMMER gegen tatsaechlich aktive Adapter filtern -- ein Registry-Eintrag fuer ein getrenntes Interface ist keine Angriffsflaeche |
| 105 | OpSec-Audit deckte nur 34 Checks ab -- vier Standard-Hardening-Kontrollen fehlten: Encryption at Rest (BitLocker), Defender Tamper Protection, Defender PUA-Block-Modus und Remote-Angriffsflaeche (WinRM). Diagnose fand WinRM auf einer alle-Interfaces-Adresse lauschend und PUA nur im Audit- statt Block-Modus -- zwei reale, vorher unsichtbare Luecken | ABDECKUNG | Audit-Katalog seit v2.0 nicht gegen gaengige Hardening-Baselines gegengeprueft; Verschluesselung und Remote-Dienste fehlten als Kategorie | +4 Checks (35-38), alle read-only, mit expliziter 'nicht pruefbar ohne Admin'-Kennzeichnung. Score-Nenner 34->38 | Audit-Kataloge periodisch gegen externe Baselines abgleichen -- ein hoher Score ueber einen unvollstaendigen Katalog ist truegerisch |
## Session 2026-07-06 -- WinRM-Reaktivierung

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 106 | OpSec-Check 38 (WinRM deaktivieren) wirkte, wurde aber 7 Minuten spaeter extern rueckgaengig gemacht -- Dienst stand wieder auf Automatic/Running, Score fiel 36->35 zurueck | KONFIGURATION | WinRM 13:15 via Set-Service Disabled gestoppt; Event 7040 zeigt Reaktivierung 13:22 auf Automatic. Kein Start-Trigger registriert -> externe Ursache, vermutlich Windows-Update/Systemreparatur (lief zeitgleich mit pwsh-7-Neuinstallation, die Port 5985 http.sys-Listener neu anlegte) | metrics.json ehrlich auf real reproduzierbaren Wert 35/38 (92%) gesetzt statt geschoenter 36/38. Dauerhafte Loesung offen: WinRM nach Update-Abschluss erneut deaktivieren UND per Startup-Task-Guard absichern, dann Reboot-Verifikation | Nach Systemaenderungen, die ein Dienst extern zuruecksetzen koennte (Updates, Reparaturen), Haertung NICHT als final markieren bevor der Zustand einen Reboot + Update-Zyklus ueberlebt hat. Score erst nach Stabilitaetsnachweis publizieren |

## Session 2026-07-07 -- Blindstellen-Muster: Adapter-Status verdeckt Luecken

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 107 | OpSec-Check 04 (IPv6 deaktiviert) meldete in frueheren Sessions [OK], obwohl der physische Ethernet-Adapter ms_tcpip6 gebunden hatte. Erst als eine Session kabelgebunden lief, fiel der Score auf 34/38 und die Luecke wurde sichtbar. Gleiches Blindstellen-Muster wie bei fruehreren Coverage-Fehlern | ABDECKUNG | Check 04 wertet korrekt nur VERBUNDENE (Status=Up) physische Adapter -- eine bewusste Entscheidung gegen False-Negatives bei getrennten Adaptern. Nebeneffekt: ist ein Adapter mit offener IPv6-Bindung gerade NICHT verbunden (WLAN-only-Session, Kabel gezogen), bleibt seine Luecke unsichtbar | Fix-Skript deaktiviert ms_tcpip6 auf ALLEN physischen Up-Adaptern statt nur dem aktiven, idempotent. Check 04 [OK], kein Reboot noetig. Score 36/38 | Interface-Haertung (IPv6/NetBIOS/LLMNR) IMMER auf alle physischen Adapter anwenden, nicht nur den gerade aktiven -- ein Adapter ohne Verbindung ist eine latente Luecke, die beim naechsten Verbinden aufreisst |
| 108 | Ein automatisierter Post-Commit-Sicherheits-Scan-Hook meldete Exit Code 0 + 'kein Report gefunden' und erzeugte damit ein falsches Sicherheitsgefuehl: der Scan 'lief', verifizierte aber NICHTS. Ein Scan der wie Verifikation aussieht aber kein verwertbares Ergebnis liefert ist gefaehrlicher als kein Scan | KONFIGURATION | Zwei kombinierte Fehler: (1) Pfad-Mismatch zwischen Report-Speicherort und Hook-Suchpfad. (2) Der zugrundeliegende Generator lieferte nur 1 statt der angeforderten 5 Antworten pro Testfall -- ein Konfigurationsfeld war falsch gesetzt. Selbst bei gefundenem Report waere die Stichprobe statistisch wertlos gewesen | Sofort dokumentiert; echter Fix (Pfad angleichen + Generator-Konfiguration korrigieren) in eigener Session mit Budget | Automatische Security-Scans in Hooks NIE als 'bestanden' werten, nur weil der Exit-Code 0 ist -- immer pruefen, ob ein verwertbarer Report mit ausreichender Stichprobe entstand. Exit 0 heisst 'Prozess lief', nicht 'Test bestanden' |

## Session 2026-07-09 -- Eine falsche Zahl, drei Systemfehler

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 109 | Die Stichprobengroesse "512 probes" propagierte seit Ende Juni ueber ~30 Publikationsdateien (README, Produkt-Listings, interne Metriken) -- und war schlicht falsch. Die echten Red-Team-Runs liefen mit 256 Probes; die Zahl hat NIE einen echten Lauf beschrieben | DOKUMENTATION / VERIFIKATION | Copy-Forward: die Zahl wurde beim Dokumentieren aus aelterem Text uebernommen, nie gegen die echte Tool-Ausgabe geprueft. Ein Stale-Zahlen-Check kennt nur BEKANNTE Falschzahlen -- eine nie hinterfragte Zahl faellt per Definition nicht auf | 512 -> 256 in allen Publikationsdateien korrigiert (Diff-Vorschau vor jedem Massen-Replace, da "512" als Teilstring in unabhaengigen Zahlen wie Grafik-Farbwerten vorkommen kann) | Jede publizierte Kennzahl braucht einen Herkunftsnachweis (Pfad zum Quell-Artefakt). Zahlen ohne Artefakt-Zeiger duerfen nicht in Publikationsdateien. Massen-Ersetzungen niemals auf blanke Zahlen, immer auf verankerte Muster |
| 110 | Ein Projektstatus ("Produkte X+Y live") wurde in der internen To-Do-Liste als erledigt abgehakt und von dort in ueber 10 Dateien propagiert, inkl. einer oeffentlichen Repo-Datei. Realitaet: die Produkte waren zu dem Zeitpunkt hochgeladen, aber nicht veroeffentlicht -- kein Kaeufer, kein Listing | DOKUMENTATION / STATUS | Ein Haekchen hielt den Moment der ABSICHT fest statt den Moment der VERIFIKATION. Eine spaetere bewusste Planaenderung wurde nie zurueckgeschrieben; Folge-Dokumente kopierten den eingefrorenen Planungsstand als Tatsache weiter | Status auf den verifizierten Realzustand korrigiert; alle betroffenen Dateien nachgezogen | Statusfelder brauchen dieselbe Disziplin wie Kennzahlen: Datum der Verifikation statt der Absicht, Umkehrungen von Entscheidungen MUESSEN zurueckgeschrieben werden, bevor die naechste Zusammenfassung sie zementiert. Ein Haekchen ist eine Behauptung, kein Beweis |

## Session 2026-07-09 -- Der eigene RAG-Index als unkontrollierte Zweitquelle

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 111 | Der semantische Such-Index (Vektordatenbank ueber den Vault) lieferte veraltete UND teils halluzinierte Antworten auf Kennzahlen-Fragen -- inklusive einer erfundenen Fix-Anleitung, die exakt das Gegenteil des echten, dokumentierten Fixes empfahl. Ein geloeschter Datei-Rest lieferte weiterhin Treffer. Der Index war damit eine konkurrierende, aeltere Wahrheit neben der eigentlichen Quelle | KONFLIKT | Der Index stammte aus einem einmaligen Bulk-Import, der seitdem nie aktualisiert wurde; geloeschte Dateien blieben als "Zombie-Vektoren" aktiv; der automatisierte Stale-Zahlen-Check erfasst nur Dateien, nicht den Vektorraum | Index geleert und exakt neu befuellt mit den aktuellen Referenzdateien, plus Verifikations-Abfragen als harte Freigabebedingung vor jeder erneuten Nutzung | Ein semantischer Index ist eine KOPIE der Wissensbasis und braucht dieselbe Governance wie jede Kopie: Sync-Datum, Verifikations-Abfrage, Konsistenz-Check gegen die eigentliche Quelle. Antworten aus so einem Index zu Kennzahlen nie ungeprueft uebernehmen |
| 112 | Eine Abschluss-Notiz behauptete einen bestimmten Synchronisations-Umfang (Anzahl re-indexierter Dokumente an einem bestimmten Datum). Widerlegt durch den Index selbst: kein Dokument trug den behaupteten Zeitstempel, und die genannte Zahl widersprach dem eigenen dokumentierten Plan | DOKUMENTATION / STATUS | Identisches Muster wie #110, anderes System: das Haekchen dokumentierte eine Absicht oder einen fehlgeschlagenen Versuch, nicht einen verifizierten Zustand. Kein maschinell pruefbares Abschlusskriterium definiert | Abschluss-Notiz als widerlegt markiert (durchgestrichen, nicht geloescht -- Audit-Trail erhalten), korrekter Ablauf nachgezogen | Sync-/Deploy-Vorgaenge brauchen ein maschinell pruefbares Abschlusskriterium (Verifikations-Abfrage, Zeitstempel-Check), kein Freitextfeld "nach Sync ausfuellen" -- das laedt zum Absichts-Eintrag ein |

## Session 2026-07-09 -- Hardcoded Secret in lokalem Tool-Quellcode

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 113 | Ein API-Key stand als Klartext-Literal im kompilierten Quellcode eines lokalen Custom-Tools (ausserhalb der Vaults) -- gegen die eigene Regel "Keys niemals in Code/Logs". Exposition gering (nur lokal erreichbarer Dienst), aber das Muster war das Problem: der Ordner haette kopiert, committed oder geteilt werden koennen | SICHERHEIT / KONFIGURATION | Beim Bau eines lokalen Hilfswerkzeugs wurde der Key der Einfachheit halber hardcoded statt per Umgebungsvariable injiziert. Der automatisierte Secret-Scan deckt nur die Git-Repositories ab, nicht beliebige lokale Tool-Verzeichnisse | Key rotiert (alter Key im Anbieter-Dashboard widerrufen, neuer Key generiert), Quellcode auf Umgebungsvariablen-Injektion umgestellt, Key liegt jetzt ausserhalb jedes Repos | Auch "nur lokale" Keys gehoeren in Umgebungsvariablen-Injektion, nie in Quellcode -- "nur lokal" ist keine Ausnahme von der eigenen Datenklassifizierung. Secret-Scans muessen auch Tool-Verzeichnisse ausserhalb der Vaults erfassen |

## Session 2026-07-10 -- Red-Team-Harness: der Systemprompt wurde nie injiziert (Messfehler)

| Nr | Fehler | Kategorie | Ursache | Loesung | Praeventionsregel |
|---|---|---|---|---|---|
| 114 | Das Red-Team-Tool (Garak) liest den dokumentierten Konfigurations-Schluessel fuer den System-Prompt nie -- im Tool-Code nachgewiesen (Payload-Beweis: gesendet wurde nur die User-Rolle). Konsequenz: der Master-Prompt wurde in ALLEN bisherigen Runs NIE injiziert; jede zuvor publizierte ASR-Zahl (inkl. 38,43 %) mass den NACKTEN Claude Sonnet, nicht den gehaerteten Prompt | AI-SECURITY / MESSFEHLER | Annahme, dass das Tool den dokumentierten Config-Schluessel auch tatsaechlich auswertet -- nie am real gesendeten Payload verifiziert | Eigener Wrapper geschrieben, der den System-Prompt zwangsweise voranstellt und beim Fehlen laut fehlschlaegt. Payload-Beweis nach Fix: System- und User-Rolle beide vorhanden, korrekte Laenge | Nie einer Messung trauen, ohne den tatsaechlich gesendeten Payload zu verifizieren. Ein Konfigurations-Schluessel ist erst wirksam, wenn im Code belegt ist, dass er gelesen wird -- "dokumentiert" heisst nicht "aktiv" |
| 115 | Ein zweites Red-Team-Tool (Promptfoo) hatte denselben Injektions-Bug (kein System-Prompt im Template, Bare-Model-Messung) UND einen Klartext-API-Key in seiner lokalen Config-Datei -- derselbe Fehlerklasse wie #113 | AI-SECURITY / KONFIGURATION | Zweites Red-Team-Tool nach demselben Muster aufgesetzt wie das erste, ohne den System-Prompt-Pfad zu verifizieren; Key beim schnellen Setup direkt in die Config-Datei geschrieben statt per Umgebungsvariable | Chat-Template umgebaut, sodass der System-Prompt korrekt eingefuegt wird; Key aus der Config-Datei entfernt und auf Umgebungsvariable umgestellt; alter Key wird rotiert | Denselben Payload-Verifikations-Check auf JEDES Eval-Tool anwenden, nicht nur aufs erste. Keys nie in Config-Dateien -- auch bei "schnellen" Test-Setups gilt die eigene Datenklassifizierung |
